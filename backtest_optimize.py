...
Tiers 1, 2 & 3 TEST. Sweeps exit targets 5%–50% in 5% steps + 11/12/13/14%.
...
def evaluate_signal(signal, bars, target_pct, hold):
    ...
    for b in bars:
        if b['high'] >= win:
            result, exit_price, exit_bar = 'WIN',  win,  b['w']; break
        if b['low']  <= stop:
            result, exit_price, exit_bar = 'LOSS', stop, b['w']; break
    if result == 'NEUTRAL':
...
def run_one_config(tickers, interval, hold, year_high_bars):
    """
    Collects all Tier 1, 2 & 3 TEST signals for the given interval/hold,
    then sweeps EXIT_TARGETS.
    Returns dict: tier -> {target -> [trades]}
    """
    cutoff   = pd.Timestamp.now() - pd.DateOffset(years=YEARS_HISTORY)
    min_bars = max(VF_LB + MAX_GAP, year_high_bars + LOOKBACK + STOCH_LOOKBACK) + hold + 10

    tier_results = {
        tier: {tgt: [] for tgt in EXIT_TARGETS}
        for tier in ['ultra', 'high', 'test']
    }
        ...
        try:
            ...
            for sig in vf_pairs:
                ...
                for tgt in EXIT_TARGETS:
                    ...
                    if t3_pass:
                        tier_results['test'][tgt].append(t)
...
def summarize_tier_for_config(tier_label, description, results_by_target, config_label):
    ...
    if summary_rows:
        ...
        for r in summary_rows:
            r['score'] = (
                (r['total_pnl'] / max_pnl if max_pnl > 0 else 0) * 0.40 +
...
def build_full_report(all_results):
    ...
    tier_configs = [
        ...
        ('test',  'TIER 3     TEST  (BB+VixFix+Stoch+Quality)',
         'Tier 2 + FCF/EPS quality filter + max 70% drawdown cap + earnings exclusion (14d).'),
    ]
    lines = [
        sep,
        'EXIT TARGET OPTIMIZATION REPORT',
        f'Tiers 1, 2 & 3 TEST | Sweep: 5%–50% (5% steps) + 11/12/13/14% granularity',
        f'Position: ${POSITION_HIGH:,.0f}/trade | History: {YEARS_HISTORY} years',
        f'Active strategies: 1wk/20-week hold | 1d/60-day hold',
        f'Tier 3 TEST filters: positive FCF or EPS growth | max 70% drawdown | earnings excl. 14d',
        sep,
        '',
    ]
...
def send_email(report):
    ...
    msg['From']    = GMAIL_USER
    msg['To']      = TO_EMAIL
    msg['Subject'] = 'Exit Target Optimization — 1wk/20w & 1d/60d — Tiers 1/2/3 TEST'
