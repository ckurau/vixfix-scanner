""" backtest.py — Weekly backtest (interval=1wk, 20-bar hold)
Run via workflow_dispatch. Emails full report with year-by-year vs SPY.
To run daily version use backtest_daily.py — identical logic, different constants.
"""
...
from tier3_filters import passes_tier3_filters
...
INTERVAL       = '1wk'
BAR_LABEL      = 'week'
YEAR_HIGH_BARS = 52        # 52 weekly bars ≈ 1 year
YEARS_HISTORY  = 15
# ══════════════════════════════════════════════════════════════════
...
HOLD_BARS    = 20
WIN_TARGET   = 0.13
POSITION_HIGH = 10000.0   # Tiers 1, 2 & 3
MIN_PRICE    = 10.0
...
def find_vixfix_pairs(df):
    ...
    for ri in range(n):
        ...
        if np.isnan(rl) or np.isnan(rw):
            continue
        if not no_break_before(lv, ri, NO_BREAK_BARS):
            continue
        ...
        for j in range(ri - 1, max(ri - MAX_GAP, 0) - 1, -1):
            ...
            if np.isnan(pl) or np.isnan(pw):
                continue
            if not no_break_before(lv, j, NO_BREAK_BARS):
                continue
            if rl < pl and rw > pw:
                pairs.append({
                    'signal_idx':  ri,
...
def find_stoch_active_set(df):
    ...
    for i in range(LOOKBACK, len(vpv)):
        if not nbv[i] or not bhlv[i]:
            continue
        if not any(vpv[max(0, i - LOOKBACK):i]):
            continue
        if not any(sdv[max(0, i - LOOKBACK):i]):
            continue
        active.add(i)
    return active
...
def eval_trade(df, signal, position_size):
    ...
    for w in range(1, HOLD_BARS + 1):
        ...
        if wh >= win:
            result, exit_price, exit_bar = 'WIN', win, w; break
        if wl <= stop:
            result, exit_price, exit_bar = 'LOSS', stop, w; break
    if result == 'NEUTRAL':
...
def run_backtest(tickers):
    ...
    results  = {k: [] for k in ['ultra', 'high', 'test']}
    ...
    for i, ticker in enumerate(tickers):
        try:
            ...
            if cc.empty:
                continue
            rc = float(cc.iloc[-1])
            if np.isnan(rc) or rc < MIN_PRICE:
                continue
            try:
            ...
            # Pre-compute rolling year high series once per ticker
            year_high_series = df['High'].rolling(YEAR_HIGH_BARS).max()

            for sig in vf_pairs:
                si, e = sig['signal_idx'], sig['entry_price']
                if e < rc * 0.15 or e > rc * 6.0:
                    continue
                if si + 1 >= len(df):
                    continue
                ...
                if not has_stoch:
                    continue   # must have stoch for Tier 2 or 3
                ...
                if sig['has_macd']:
                    results['ultra'].append(t)
                results['high'].append(t)

                # ── Tier 3 TEST: Tier 2 signal + quality filters ──────────
                sig_date  = df.index[si]
                sig_date  = sig_date.date() if hasattr(sig_date, 'date') else sig_date
                year_high = float(year_high_series.iloc[si]) if not np.isnan(year_high_series.iloc[si]) else 0.0
                if passes_tier3_filters(
                    ticker        = ticker,
...
def tier_summary_block(label, s, spm):
    ...
    return (
        f'  {label}\n'
        f'    Signals: {s["total"]:5d}  ~{spm:.1f}/mo'
        f'  Wins: {s["wins"]:4d} ({s["wr"]:.1f}%)'
        f'  Losses: {s["losses"]:4d} ({s["lr"]:.1f}%)'
        f'  Neutral: {s["neutral"]:4d}\n'
        f'    Avg win: {s["aw"]:+.1f}%  Avg loss: {s["al"]:+.1f}%'
        f'  EV: {s["ev"]:+.2f}%  Avg hold:{s["hold_b"]:.1f}{bl}\n'
        f'    Total P&L: $ {s["pnl"]:>+14,.2f}  ROI: {s["roi"]:>+6.1f}%\n'
    )
...
def write_stats_json(results, stats_path='backtest_stats.json'):
    ...
    # Merge — preserve daily_60d stats written by backtest_daily.py
    existing = {}
...
def build_report(results):
    ...
    tiers = [
        ...
        ('test',  POSITION_HIGH, 'TIER 3     TEST     (BB+VixFix+Stoch+Quality)'),
    ]
    lines = [
        sep,
        ...
        f'  Position: ${POSITION_HIGH:,.0f}/trade (all tiers)',
        f'  Filters: Price >${MIN_PRICE:.0f} | Mkt cap >$1B | Stop dist <{int(MAX_STOP_DIST*100)}%',
        f'  Tier 3 adds: FCF/EPS quality + max 70% drawdown cap + earnings exclusion (14d)',
        f'  ExpVal = (Win% x Avg Win%) + (Loss% x Avg Loss%)',
        f'  ROI = Total P&L / (Signals x Position Size)',
        sep,
        '',
        '── TIER COMPARISON ──────────────────────────────────────────────────────────────',
    ]
...
if __name__ == '__main__':
    ...
    if GMAIL_USER:
        send_email(
            f'Backtest Report — {INTERVAL.upper()} — {HOLD_BARS}-{BAR_LABEL} hold',
            report
        )
