"""
scanner_combined.py  —  Weekly scanner (interval=1wk, Tiers 1 & 2 only)
Reads historical stats from backtest_stats.json (written by backtest.py).
Run backtest.py first via workflow_dispatch to populate stats.
Sends HTML email with full tier stats + red ticker symbols.
Entry logic: buy at next Monday open after Friday scan completes.
"""
...
HOLD_BARS      = 20;  WIN_TARGET = 0.13;  POSITION_HIGH = 10000.0
...
# ── Load backtest stats from JSON ──────────────────────────────────────────────
def load_backtest_stats():
    """Read stats written by backtest.py. Returns dict with 'ultra' and 'high' keys."""
    try:
...
def run_scans(tickers):
    ...
    print(f'Scanning {len(tickers)} tickers (weekly)...\n')
    for i, ticker in enumerate(tickers):
        try:
            df = yf.download(ticker, period='3y', interval=INTERVAL,
...
# ── HTML report ────────────────────────────────────────────────────────────────
def _stats_html(s):
    """Render the full stats block from backtest JSON into HTML rows."""
    if not s:
        return ('<p style="margin:6px 0;font-size:0.85em;color:#c00;">'
                '⚠ No backtest data — run <strong>backtest.py</strong> via '
...
<table style="border-collapse:collapse;width:100%;font-size:0.82em;margin:6px 0;">
  ...
  <tr>
    ...
    <td style="padding:3px 8px;">{s["hold_b"]:.1f} weeks</td>
...
def _tier_html(title, description, tickers_this_tier, stats, hide_already_in=None):
    ...
    if show:
        ...
        ticker_html = '<em style="color:#999;">No signals this week.</em>'
...
<div style="border:1px solid #ccc;border-radius:6px;padding:14px 16px;
  ...
  <p style="margin:2px 0;font-size:0.78em;color:#777;">
    Strategy: Buy at <strong>Monday open</strong> after Friday signal &nbsp;|&nbsp;
    Stop: low of BB trigger candle &nbsp;|&nbsp;
    Win target: {int(WIN_TARGET*100)}% &nbsp;|&nbsp; Max hold: {HOLD_BARS} weeks
  </p>
  {_stats_html(stats)}
...
<p style="margin:0 0 4px 0;font-size:0.82em;color:#666;">
  Interval: Weekly (1wk) &nbsp;|&nbsp; Universe: NYSE + NASDAQ &nbsp;|&nbsp;
  Price &gt;$10, Mkt cap &gt;$1B, Stop dist &lt;11%
</p>
<p style="margin:0 0 16px 0;font-size:0.82em;color:#555;">
  <strong>Timing:</strong> Signal fires after Friday close.
  Buy at <strong>Monday market open</strong>. Entry = Friday close of BB trigger candle.
</p>
<h3 style="margin:0 0 6px 0;font-size:0.95em;">Signal Count This Week</h3>
<table><tr><th>Tier</th><th>Signals</th></tr>{summary}</table>
{ts}
<p style="font-size:0.72em;color:#aaa;margin-top:16px;">
  Backtest stats sourced from backtest_stats.json (written by backtest.py).
  Re-run backtest.py via workflow_dispatch to refresh.
</p>
</body></html>'''
...
if __name__ == '__main__':
    ...
    if GMAIL_USER:
        ...
        subject = (f'Weekly Stock Scan (1wk) — {date.today().strftime("%b %d %Y")} '
