""" backtest_daily.py — Daily (1d) backtest, Tiers 1, 2 & 3 TEST, 60-day hold.
...
def build_report(results):
    ...
    tiers = [
        ...
        ('test',  'TIER 3     TEST     (BB+VixFix+Stoch+Quality)  '),
    ]
    lines = [
        sep,
        f'BACKTEST REPORT — DAILY (1d) | {YEARS_HISTORY} years | '
        f'{HOLD_BARS}-day hold | {int(WIN_TARGET*100)}% win target',
        sep,
        ...
        f'  ROI = Total P&L / (Signals x ${POSITION_HIGH:,.0f})',
        sep,
        '',
        '── TIER COMPARISON ──────────────────────────────────────────────────────────────',
    ]
